# ============================================================
# OPTIMIZED PYTORCH BRAIN TUMOR CLASSIFIER (RTX 3080)
# Apple-to-apple: batch_size_inference = 16
#
# Key updates vs your last version:
# ✅ Fixed signature mismatch (removed confidence_threshold usage)
# ✅ Inference speed stack for RTX 3080:
#    - torch.compile(mode="max-autotune-no-cudagraphs")
#    - channels_last (inference)
#    - CUDA Graphs for PRUNED inference when dynamic_kmax=False (fixed path)
# ✅ CUDA-graph-safe ROI path (no Python data-dependent if all_zero.any())
#
# ✅ NOVELTY ADDITION (minimal change, main logic unchanged):
#    - Calibration-aware constraint via Differentiable "Soft-ECE" regularizer
#    - Also reports standard (hard) ECE metric on val for FULL and PRUNED
#
# ✅ NEW (YOUR REQUEST, MINIMAL EDITS):
#    - Collect run metrics + speed and append one row into a CSV:
#      modality, model, best_epoch, best_by, val_acc, val_macro_p, val_macro_r,
#      val_macro_f1, val_w_f1, val_auc_macro_ovr, val_mean_confidence,
#      val_ece_15bins, val_nll, val_brier, params_m, ms_per_img, throughput_img_s
# ============================================================
...
seed = 0
batch_size = 16
batch_size_inference = 16   # apple-to-apple fixed
image_size = 192
patch_size = 112
...
dynamic_kmax = False       # keep False if you want CUDA Graph benefit
...
# -----------------------
# NEW: CALIBRATION-AWARE CONSTRAINT (Soft-ECE regularizer)
# Keep weight small to avoid hurting main optimization.
# Set ece_weight=0.0 to disable without touching code.
# -----------------------
...
# Paths
dataset = "T2"
train_dir = r"/home/nirob/Desktop/Brain tumor mri /dataset/data_split/train/T2"
val_dir   = r"/home/nirob/Desktop/Brain tumor mri /dataset/data_split/val/T2"
CKPT_SAVE_PATH = r"/home/nirob/Desktop/Brain tumor mri /checkpoints/t2_final_best_optimized_model.pth"
...
# -----------------------
def main():
    # NOTE: This main() assumes you already have these defined ABOVE:
    # - run_training, evaluate_full, evaluate_pruned_fast, evaluate_pruned_tta
    # - compute_ece_metric, benchmark_inference_val, hard_pruned_predict_batch_fast
    # - CUDAGraphRunner
    # - collect_val_probs, auc_macro_ovr, brier_multiclass, _append_row_csv
    # - METRICS_CSV_PATH, class_names, num_classes, train_dir
    # - train_loader/val_loader/val_loader_fast, CKPT_SAVE_PATH
    # - and imports: Path, precision_recall_fscore_support, f1_score, log_loss
    ...
    #     # dynamic thresholds (only if dynamic_kmax=True)
    #     "entropy_thr": None,
    #     "margin_thr": 0.15,
    ...
    #     # calibration-aware constraint knobs
    ...
    # # torch.compile (RTX 3080-friendly)
    # if device.type == "cuda":
    #     try:
    ...
    # # ---------------- CUDA Graph for PRUNED inference (fixed path) ----------------
    # graph_runner = None
    ...
    #             tau=cfg["tau_infer"],
    #             Kmax=cfg["Kmax_infer"],
    #             use_dynamic=False,  # fixed path for graphs
    #             entropy_thr=cfg.get("entropy_thr", None),
    #             margin_thr=cfg.get("margin_thr", 0.15),
    #             print_escalation=False,
    #             per_sample_escalation=True,
    ...
    # # IMPORTANT: force 15 bins for the column name "val_ece_15bins"
    # ece_full = compute_ece_metric(
    #     model, val_loader, cfg,
    #     pruned=False, graph_runner=None, n_bins=15
    # )

    # print(f"Accuracy: {acc_full:.4f} ({acc_full*100:.2f}%)")
    # print(f"ECE (15 bins): {ece_full:.4f}")
    ...
    # # ---------------- EVAL PRUNED ----------------
    # print("\n" + "=" * 70)
    # print("⚡ PRUNED MODEL EVALUATION (FAST)")
    # print("=" * 70)

    # y_true_p, y_pred_p = evaluate_pruned_fast(
    #     model, val_loader_fast, cfg,
    #     graph_runner=graph_runner, max_steps=None
    # )
    # acc_pruned = accuracy_score(y_true_p, y_pred_p)

    # ece_pruned = compute_ece_metric(
    #     model, val_loader_fast, cfg,
    #     pruned=True, graph_runner=graph_runner, n_bins=15
    # )

    # print(f"Accuracy: {acc_pruned:.4f} ({acc_pruned*100:.2f}%)")
    # print(f"ECE (15 bins): {ece_pruned:.4f}")
    # print("\nClassification Report:")
    ...
    # if use_tta:
    #     print("\n" + "=" * 70)
    #     print("🎯 PRUNED MODEL WITH TTA")
    #     print("=" * 70)
    #     y_true_tta, y_pred_tta = evaluate_pruned_tta(model, val_loader, cfg, n_augments=4, max_steps=None)
    #     acc_tta = accuracy_score(y_true_tta, y_pred_tta)
    #     print(f"Accuracy: {acc_tta:.4f} ({acc_tta*100:.2f}%)")
    #     print("\nClassification Report:")
    #     print(classification_report(y_true_tta, y_pred_tta, target_names=class_names, digits=4, zero_division=0))
    ...
    # # ---------------- BENCHMARK FULL (for CSV logging) ----------------
    ...
    # # =================================================================
    # # CSV LOGGING: write TWO rows per run (FULL + PRUNED)
    # # =================================================================
    # modality = Path(train_dir).name  # e.g., "T1C+"
    ...
    # def _metrics_from_probs(y_true_arr: np.ndarray, probs_arr: np.ndarray):
    #     if probs_arr.size == 0:
    #         return {
    #             "val_acc": float("nan"),
    ...
    #     val_w_f1 = float(f1_score(y_true_arr, y_pred_arr, average="weighted", zero_division=0))
    #     val_auc_macro_ovr = float(auc_macro_ovr(y_true_arr, probs_arr))

    #     conf = probs_arr.max(axis=1)
    #     val_mean_confidence = float(np.mean(conf))

    #     val_nll = float(log_loss(y_true_arr, probs_arr, labels=np.arange(num_classes)))
    #     val_brier = float(brier_multiclass(y_true_arr, probs_arr, n_classes=num_classes))
    ...
    # # -------- FULL row --------
    # y_true_full_probs, probs_full = collect_val_probs(
    #     model, val_loader_fast, cfg,
    #     pruned=False, graph_runner=None, max_steps=None
    # )
    # m_full = _metrics_from_probs(y_true_full_probs, probs_full)

    # row_full = {
    #     "modality": modality,
    #     "model": base_model_name + "|full",
    #     "best_epoch": best_epoch,
    #     "best_by": best_by,

    #     **m_full,
    #     "val_ece_15bins": float(ece_full),
    ...
    # # -------- PRUNED row --------
    # y_true_pr_probs, probs_pr = collect_val_probs(
    #     model, val_loader_fast, cfg,
    #     pruned=True, graph_runner=graph_runner, max_steps=None
    # )
    # m_pr = _metrics_from_probs(y_true_pr_probs, probs_pr)

    # row_pruned = {
    #     "modality": modality,
    #     "model": base_model_name + f"|pruned(Kmax={cfg['Kmax_infer']},tau={cfg['tau_infer']},dyn={cfg['dynamic_kmax']})",
    #     "best_epoch": best_epoch,
    #     "best_by": best_by,

    #     **m_pr,
    #     "val_ece_15bins": float(ece_pruned),
    ...
    # print(f"\n🧾 CSV metrics appended (FULL + PRUNED) -> {METRICS_CSV_PATH}")

    # # ---------------- SUMMARY ----------------
    # print("\n" + "=" * 70)
    # print("📈 FINAL RESULTS SUMMARY")
    # print("=" * 70)
    # print(f"Full Model Accuracy     : {acc_full*100:.2f}%")
    # print(f"Full Model ECE (15)     : {ece_full:.4f}")
    # print(f"Pruned Model Accuracy   : {acc_pruned*100:.2f}%")
    # print(f"Pruned Model ECE (15)   : {ece_pruned:.4f}")
    # if use_tta:
    #     print(f"Pruned + TTA Accuracy   : {acc_tta*100:.2f}%")
    ...
    print("="*70)
    print("Config (key):")
    for k in ["K_rois","Kmax_infer","dynamic_kmax","tau_infer","gate_temperature","target_K","budget_weight","lr_stage1","lr_stage2",
              "ece_weight","ece_bins","ece_smoothing","ece_temperature"]:
        print(f"  {k}: {cfg[k]}")
    print("="*70)
    ...
    y_true, y_pred = evaluate_full(model, val_loader, cfg, max_steps=None)
    acc_full = accuracy_score(y_true, y_pred)
    ece_full = compute_ece_metric(model, val_loader, cfg, pruned=False, graph_runner=None, n_bins=15)
    ece_pruned = compute_ece_metric(model, val_loader_fast, cfg, pruned=True, graph_runner=graph_runner, n_bins=15)


    print(f"Accuracy: {acc_full:.4f} ({acc_full*100:.2f}%)")
    print(f"ECE ({cfg['ece_bins']} bins): {ece_full:.4f}")
    print("\nClassification Report:")
    print(classification_report(y_true, y_pred, target_names=class_names, digits=4, zero_division=0))
    print("\nConfusion Matrix:")
    print(confusion_matrix(y_true, y_pred))
    ...
    acc_pruned = accuracy_score(y_true_p, y_pred_p)
    ece_pruned = compute_ece_metric(model, val_loader_fast, cfg, pruned=True, graph_runner=graph_runner, n_bins=cfg["ece_bins"])

    print(f"Accuracy: {acc_pruned:.4f} ({acc_pruned*100:.2f}%)")
    print(f"ECE ({cfg['ece_bins']} bins): {ece_pruned:.4f}")
    print("\nClassification Report:")
    print(classification_report(y_true_p, y_pred_p, target_names=class_names, digits=4, zero_division=0))
    print("\nConfusion Matrix:")
    print(confusion_matrix(y_true_p, y_pred_p))
    ...
    # ---------------- >>> NEW: CSV LOGGING (one row append) ----------------
    # We log PRUNED-val metrics + PRUNED speed (typical deployment mode).
    modality = Path(train_dir).name  # e.g., "T1C+"
    model_name = f"STNROI+{BACKBONE_NAME}|pruned(Kmax={cfg['Kmax_infer']},tau={cfg['tau_infer']},dyn={cfg['dynamic_kmax']})"
    ...
    # collect probabilities for AUC/NLL/Brier/mean_conf (extra forward pass; no logic change)
    y_true_probs, probs = collect_val_probs(
        model, val_loader_fast, cfg, pruned=True, graph_runner=graph_runner, max_steps=None
    )
    if probs.size > 0:
        y_pred_probs = probs.argmax(axis=1)
        val_acc = float(accuracy_score(y_true_probs, y_pred_probs))

        p_macro, r_macro, f1_macro, _ = precision_recall_fscore_support(
            y_true_probs, y_pred_probs, average="macro", zero_division=0
        )
        val_macro_p = float(p_macro)
        val_macro_r = float(r_macro)
        val_macro_f1 = float(f1_macro)

        val_w_f1 = float(f1_score(y_true_probs, y_pred_probs, average="weighted", zero_division=0))
        val_auc_macro_ovr = float(auc_macro_ovr(y_true_probs, probs))

        conf = probs.max(axis=1)
        val_mean_confidence = float(np.mean(conf))

        val_nll = float(log_loss(y_true_probs, probs, labels=np.arange(num_classes)))
        val_brier = float(brier_multiclass(y_true_probs, probs, n_classes=num_classes))
    else:
        val_acc = float("nan")
        val_macro_p = float("nan")
        val_macro_r = float("nan")
    ...
    val_ece_15bins = float(ece_pruned)  # already computed above with cfg["ece_bins"]=15
    ...
    _append_row_csv(METRICS_CSV_PATH, fieldnames, row)
    print(f"\n🧾 CSV metrics appended -> {METRICS_CSV_PATH}")
    ...
    print("="*70)
    print(f"Full Model Accuracy     : {acc_full*100:.2f}%")
    print(f"Full Model ECE          : {ece_full:.4f}")
    print(f"Pruned Model Accuracy   : {acc_pruned*100:.2f}%")
    print(f"Pruned Model ECE        : {ece_pruned:.4f}")
    if use_tta:
        print(f"Pruned + TTA Accuracy   : {acc_tta*100:.2f}%")
    print(f"Inference Throughput    : {bench_pruned['throughput_img_per_s']:.2f} images/s")
    print("="*70)
...
# ============================================================
# OPTUNA TUNING (optional)  [UNCHANGED]
# ============================================================
...
def _cfg_from_trial(trial):
    ...
    if TUNE_FAST_MODE:
        ...
        cfg["ece_weight"] = trial.suggest_float("ece_weight", 0.0, 0.08)          # example range
        cfg["ece_smoothing"] = trial.suggest_float("ece_smoothing", 0.005, 0.06)  # example range
        cfg["ece_temperature"] = trial.suggest_float("ece_temperature", 0.7, 2.0) 

    # cfg["K_rois"] = trial.suggest_int("K_rois", 1, 4)
    # cfg["lr_stage1"] = trial.suggest_float("lr_stage1", 5e-5, 5e-4, log=True)
    # cfg["lr_stage2"] = trial.suggest_float("lr_stage2", 1e-5, 2e-4, log=True)

    # cfg["target_K"] = trial.suggest_float("target_K", 0.0, float(cfg["K_rois"]))
    # cfg["budget_weight"] = trial.suggest_float("budget_weight", 1e-4, 0.5, log=True)
    # cfg["gate_temperature"] = trial.suggest_float("gate_temperature", 0.3, 2.0)

    # cfg["label_smoothing"] = trial.suggest_float("label_smoothing", 0.0, 0.2)
    # cfg["mixup_alpha"] = trial.suggest_float("mixup_alpha", 0.0, 0.6)
    # cfg["ema_decay"] = trial.suggest_float("ema_decay", 0.95, 0.9999)

    # cfg["warmup_epochs"] = trial.suggest_int("warmup_epochs", 0, 8)
    # cfg["patience"] = trial.suggest_int("patience", 8, 20)

    # cfg["tau_infer"] = trial.suggest_float("tau_infer", 0.05, 0.95)
    # cfg["Kmax_infer"] = trial.suggest_int("Kmax_infer", 0, int(cfg["K_rois"]))
    # cfg["dynamic_kmax"] = trial.suggest_categorical("dynamic_kmax", [False, True])

    return cfg

def _pick_best_pareto(best_trials, target_acc=0.98, target_thr=2300.0):
    best = None
    best_score = float("inf")
    for t in best_trials:
        acc, thr = t.values
        da = max(0.0, target_acc - acc) / max(target_acc, 1e-12)
        dt = max(0.0, target_thr - thr) / max(target_thr, 1e-12)
        score = da * da + dt * dt
        if score < best_score:
            best_score = score
            best = t
...
def tune_main():
    ...
    def objective(trial):
        ...
        try:
            ...
            ece_pruned = compute_ece_metric(
            model, val_loader_fast, cfg,
            pruned=True, graph_runner=None, n_bins=15
            )
            trial.set_user_attr("ece_pruned_15", float(ece_pruned))
            ...
            if TUNE_ENABLE_MULTI_OBJECTIVE:
                return acc_pruned, thr, float(ece_pruned)
            else:
                return float(acc_pruned + 1e-4 * thr)
    ...
    if TUNE_ENABLE_MULTI_OBJECTIVE:
        ...
        best = _pick_best_pareto(study.best_trials, target_acc=0.98, target_thr=2300.0)
        print("\n Chosen best (closest to target 98% & 2300 img/s):")
        print("  values(acc, thr):", best.values)
